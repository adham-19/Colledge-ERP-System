...
export class FacultyProfileUpdateComponent implements OnInit {
  ...
  submit(): void {
    const { name, dob, department, contactNumber, avatar, designation } =
      this.value;
    if (
      !name &&
      !dob &&
    ...
    this.loading = true;
    this.error = "";
    this.facultyService.updateProfile(this.value).subscribe({
      next: (res) => {
        this.loading = false;
        this.authService.updateCurrentUser(res);
        this.router.navigateByUrl("/faculty/profile");
      },
      error: (err) => {
        this.loading = false;
